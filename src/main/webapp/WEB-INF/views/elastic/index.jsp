...
<div class="content">
	<div class="block block-rounded">
		<div class="block-content">
			<div class="table-responsive">
				...
				<form>
					<input type="hidden" name="search" value="${index.str }">
					<c:choose>
						<c:when test="${index.len > 0}">
							<div>
								<div>	
									<a href="index?search=${index.str}&category=${value}" value="JTBC"><strong class="lead">JTBC</strong></a>
	...
	<%-- 										<fmt:parseDate value="${jtbc.date}" var="date" pattern="yyyyMMdd"/> --%>
									...
									<c:if test="${index.elastic.stotal.item0 > 4 }">
										<a href="javascript:void(0)" value="JTBC" value="JTBC" class="valueMore" onclick="pageSubmitFn(${jtbc.violt_ctgr_cd_nm})"><span>JTBC 기사 더보기</span></a>
									</c:if>
								<hr />
								</div>
								<div>
									<a href="index?search=${index.str}&category=${value}" value="KBS"><strong class="lead">KBS</strong></a>
									...
									<c:if test="${index.elastic.stotal.item1 > 4 }">
										<input type="hidden" value="KBS" name="Category">
										<input type="submit" value="KBS 기사 더보기" onclick="location.href='index?search=${index.str}&category=${index.Category}'">
									</c:if>
								<hr />
								</div>
								<div>
									<a href="index?search=${index.str}&category=${index.JTBC}"><strong class="lead">MBC</strong></a>
									...
									<input type="button" value="MBC 기사 더보기" onclick="location.href='index?search=${index.str}&category=MBC'">
									</c:if>
								<hr />
								</div>
							</div>
